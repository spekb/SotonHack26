...
function FieldBox({
  label, value, active, success, placeholder,
}: {
  label: string; value?: string; active?: boolean; success?: boolean; placeholder?: string;
}) {
  const border = success ? "1.5px solid var(--accent-green)"
    : active ? "1.5px solid var(--accent-blue)"
    : "0.5px solid var(--border-subtle)";
  const lc = success ? "var(--accent-green)" : active ? "var(--accent-blue)" : "var(--text-faint)";
  return (
    <div style={{ background: "var(--bg-secondary)", border, borderRadius: 10, padding: "12px 14px", marginBottom: 10 }}>
      <p style={{ fontSize: 9, color: lc, fontWeight: 600, letterSpacing: "0.05em", marginBottom: 3 }}>{label}</p>
      <p style={{ fontSize: 13, color: value ? "var(--text-primary)" : "var(--text-hint)" }}>
        {value || placeholder || ""}
      </p>
    </div>
  );
  ...
  return (
    <div style={{ animation: "fadeIn 0.25s ease" }}>
      ...
      <div style={{ background: "var(--bg-quaternary)", borderRadius: 8, padding: "10px 13px", marginBottom: 16 }}>
        <p style={{ fontSize: 9, color: "var(--text-faint)", marginBottom: 6, fontWeight: 600 }}>PASSWORD STRENGTH</p>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, margin: "14px 0" }}>
        ...
        <span style={{ fontSize: 10, color: "var(--text-faint)" }}>or sign up with</span>
        <div style={{ flex: 1, height: "0.5px", background: "var(--border-subtle)" }} />
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
        {["Google","GitHub","Apple"].map(s => (
          <button key={s} style={{
            ...
            borderRadius: 10, padding: "11px", cursor: "pointer", fontFamily: "inherit",
...
───────────────────────────────────────── */
function StepNative({
  native, setNative, onNext, onBack,
}: {
  ...
  return (
    <div style={{ animation: "fadeIn 0.25s ease" }}>
      <p style={{ fontSize: 10, color: "var(--text-faint)", fontWeight: 600, letterSpacing: "0.06em", marginBottom: 6 }}>STEP 2 OF 5</p>
      <h2 style={{ fontSize: 22, fontWeight: 700, marginBottom: 4 }}>What's your native language?</h2>
        ...
        {filtered.map(l => {
          ...
          return (
            <div key={l.name} onClick={() => setNative(l.name)} style={{
              ...
              <span style={{ fontSize: 13, color: sel ? "var(--text-primary)" : "var(--text-muted)", fontWeight: sel ? 600 : 400, flex: 1 }}>
                {l.name}
              </span>
              {sel && (
                <div style={{
...
───────────────────────────────────────── */
function StepLearn({
  native, learn, setLearn, onNext, onBack,
}: {
  native: string; learn: string; setLearn: (v: string) => void;
  onNext: () => void; onBack: () => void;
}) {
  const [search, setSearch] = useState("");
...
function StepSkill({
  learnLang, cefr, setCefr, duoScore, setDuoScore, onNext, onBack,
}: {
  learnLang: string;
  cefr: CEFRLevel; setCefr: (v: CEFRLevel) => void;
  duoScore: number; setDuoScore: (v: number) => void;
  onNext: () => void; onBack: () => void;
}) {
  ...
  const methodBtnStyle = (m: SkillMethod): React.CSSProperties => ({
    ...
    background: method === m
      ? m === "cefr" ? "var(--accent-blue-bg)"
        : m === "duolingo" ? "#0f2200"
        : "var(--accent-orange-bg)"
      : "var(--bg-secondary)",
    color: method === m
      ? m === "cefr" ? "var(--accent-blue)"
        : m === "duolingo" ? "#58cc02"
        : "var(--accent-orange)"
      : "var(--text-muted)",
  ...
  return (
    <div style={{ animation: "fadeIn 0.25s ease" }}>
      <p style={{ fontSize: 10, color: "var(--text-faint)", fontWeight: 600, letterSpacing: "0.06em", marginBottom: 6 }}>STEP 4 OF 5</p>
      <h2 style={{ fontSize: 22, fontWeight: 700, marginBottom: 4 }}>Your {learnLang} level</h2>
      <p style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 20 }}>
        ...
        <div style={{ animation: "fadeIn 0.2s ease" }}>
            ...
            {CEFR_ORDER.map(l => {
              ...
              return (
                <button key={l} onClick={() => setCefr(l)} style={{
                  flex: 1, borderRadius: 9, padding: "12px 4px", textAlign: "center",
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
              <div style={{ background: cd.color, borderRadius: 7, padding: "4px 10px", display: "inline-flex" }}>
                <span style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>{cefr}</span>
              </div>
              <span style={{ fontSize: 14, fontWeight: 700, color: cd.color }}>{cd.label}</span>
              <span style={{ fontSize: 11, color: "var(--text-faint)", marginLeft: "auto" }}>{cd.short}</span>
            </div>
          ...
          <div style={{ background: "var(--bg-secondary)", borderRadius: 10, padding: "12px 14px", marginBottom: 4 }}>
            ...
            <div style={{ position: "relative", height: 8, background: "var(--bg-tertiary)", borderRadius: 4, marginBottom: 8 }}>
              <div style={{
                ...
                width: `${(CEFR_ORDER.indexOf(cefr) + 1) / 6 * 100}%`,
                background: cd.color, transition: "width 0.4s ease",
              }} />
            </div>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              {CEFR_ORDER.map((l) => (
                <span key={l} style={{
                  fontSize: 9, fontWeight: l === cefr ? 700 : 400,
                  color: l === cefr ? cd.color : "var(--text-hint)",
                }}>{l}</span>
              ))}
            </div>
          </div>
      ...
      {method === "duolingo" && (
        <div style={{ animation: "fadeIn 0.2s ease" }}>
          ...
          <div style={{
            ...
            <p style={{ fontSize: 11, color: "#a0e060", lineHeight: 1.5 }}>
          ...
          <div style={{
            ...
            <div style={{ textAlign: "center", marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "center", gap: 6 }}>
                <span style={{
                  fontSize: 56, fontWeight: 800, color: band.color, lineHeight: 1,
                  transition: "color 0.3s ease",
                }}>{duoScore}</span>
                <span style={{ fontSize: 16, color: "var(--text-faint)", marginBottom: 8 }}>/160</span>
              </div>
              <div style={{
                display: "inline-flex", alignItems: "center", gap: 6, marginTop: 6,
                background: "var(--bg-quaternary)", borderRadius: 20,
                padding: "4px 14px", border: `0.5px solid ${band.color}40`,
              }}>
                <div style={{ width: 7, height: 7, borderRadius: "50%", background: band.color }} />
                <span style={{ fontSize: 12, color: band.color, fontWeight: 700 }}>{band.label}</span>
                <span style={{ fontSize: 10, color: "var(--text-faint)" }}>{band.desc}</span>
              </div>
            </div>
            ...
            <div style={{ position: "relative", marginBottom: 6 }}>
              ...
              <div style={{ height: 6, borderRadius: 3, overflow: "hidden", display: "flex", marginBottom: 8 }}>
                {[
                  ...
                  <div key={i} style={{
                    width: `${seg.w}%`, height: "100%",
                    background: seg.color,
                    opacity: duoScore >= [0,20,50,85,115,140][i] ? 1 : 0.25,
          ...
          <div style={{
            ...
            <div>
              <p style={{ fontSize: 9, color: "var(--text-faint)", fontWeight: 600, marginBottom: 6 }}>EQUIVALENT CEFR LEVEL</p>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ background: scoreCd.color, borderRadius: 6, padding: "4px 10px" }}>
                  <span style={{ fontSize: 14, fontWeight: 800, color: "#fff" }}>{scoreCefr}</span>
                </div>
                <span style={{ fontSize: 13, color: scoreCd.color, fontWeight: 700 }}>{scoreCd.label}</span>
              </div>
            </div>
            <p style={{ fontSize: 11, color: "var(--text-muted)", lineHeight: 1.5, marginLeft: "auto", maxWidth: 160, textAlign: "right" }}>
              {scoreCd.short}
            </p>
          </div>
        </div>
...
function StepConfirm({
  ...
  return (
    <div style={{ animation: "fadeIn 0.25s ease" }}>
      <p style={{ fontSize: 10, color: "var(--text-faint)", fontWeight: 600, letterSpacing: "0.06em", marginBottom: 6 }}>STEP 5 OF 5</p>
      <h2 style={{ fontSize: 22, fontWeight: 700, marginBottom: 4 }}>You're all set!</h2>
      ...
      <div style={{
        ...
        {/* Language pair */}
        <div style={{ padding: "16px 18px", borderBottom: "0.5px solid var(--border-subtle)" }}>
          <p style={{ fontSize: 9, color: "var(--text-faint)", fontWeight: 600, marginBottom: 12 }}>LANGUAGE PAIR</p>
          <div style={{ display: "flex", alignItems: "center", gap: 0 }}>
            <div style={{
              flex: 1, background: "var(--accent-blue-bg)", border: "0.5px solid var(--accent-blue-border)",
              borderRadius: "10px 0 0 10px", padding: "12px 14px", textAlign: "center",
            }}>
              <span style={{ fontSize: 24 }}>{nFlag}</span>
              <p style={{ fontSize: 12, fontWeight: 700, color: "var(--accent-blue)", marginTop: 4 }}>{native}</p>
              <p style={{ fontSize: 9, color: "var(--text-faint)" }}>I speak</p>
            </div>
            <div style={{
              width: 32, height: 32, borderRadius: "50%",
              background: "var(--bg-tertiary)", border: "0.5px solid var(--border-subtle)",
              display: "flex", alignItems: "center", justifyContent: "center",
              fontSize: 14, color: "var(--text-muted)", zIndex: 1, flexShrink: 0,
            }}>⇄</div>
            <div style={{
              flex: 1, background: "var(--accent-green-bg)", border: "0.5px solid var(--accent-green-border)",
              borderRadius: "0 10px 10px 0", padding: "12px 14px", textAlign: "center",
            }}>
              <span style={{ fontSize: 24 }}>{lFlag}</span>
              <p style={{ fontSize: 12, fontWeight: 700, color: "var(--accent-green)", marginTop: 4 }}>{learn}</p>
              <p style={{ fontSize: 9, color: "var(--text-faint)" }}>I'm learning</p>
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: "14px 18px" }}>
          <p style={{ fontSize: 9, color: "var(--text-faint)", fontWeight: 600, marginBottom: 12 }}>SKILL LEVEL</p>
          <div style={{ display: "flex", gap: 10 }}>
            <div style={{
              flex: 1, background: cd.bg, border: `0.5px solid ${cd.border}`,
              borderRadius: 10, padding: "12px 14px",
            }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                <div style={{ background: cd.color, borderRadius: 6, padding: "2px 8px" }}>
                  <span style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>{cefr}</span>
                </div>
                <span style={{ fontSize: 12, fontWeight: 700, color: cd.color }}>{cd.label}</span>
              </div>
              <p style={{ fontSize: 10, color: "var(--text-muted)" }}>{cd.short}</p>
            </div>
            {duoScore > 0 && (
              <div style={{
                ...
                <p style={{ fontSize: 22, fontWeight: 800, color: band.color, marginTop: 4, lineHeight: 1 }}>
                  {duoScore}
                </p>
                <p style={{ fontSize: 9, color: "var(--text-faint)", marginTop: 2 }}>/ 160</p>
                <p style={{ fontSize: 9, color: band.color, fontWeight: 700, marginTop: 3 }}>{band.label}</p>
              </div>
            )}
          </div>
        </div>
      ...
      {/* Matching note */}
      <div style={{
        background: "var(--accent-blue-bg)", border: "0.5px solid var(--accent-blue-border)",
    ...
    <div style={{
      ...
      <div style={{ width: "100%", maxWidth: 520 }}>
        ...
        <div style={{
          ...
          <Link href="/dashboard" style={{ fontSize: 14, fontWeight: 700, color: "var(--text-primary)", textDecoration: "none" }}>
            LinguaLink
          </Link>
        ...
        <div style={{
          ...
          {step === 3 && (
            <StepLearn
              native={native} learn={learn} setLearn={setLearn}
              onNext={() => setStep(4)} onBack={() => setStep(2)}
            />
          )}
          {step === 4 && (
            <StepSkill
              learnLang={learn || "your target language"}
              cefr={cefr} setCefr={setCefr}
              duoScore={duoScore} setDuoScore={setDuoScore}
